...
async def pv_filter(client, message):
    ...
    if settings['BOT_FFILTER']:
        kd = await pmauto_filter(client, message) 
        if kd == False:
...
async def next_page(bot, query):
    ...
    try:
        if settings['auto_delete']:
            ...
            btn.insert(1, 
            [
                InlineKeyboardButton(f'ဝင်မရပါက ဤနေရာနှိပ်၍ Channel များ Join ပါ', url=f"{Script.ALL_LINK}")                
            ]
            )
    ...
    btn.insert(0, [
        InlineKeyboardButton(f'🇩 🇴 🇳 🇦 🇹 🇪', url=f"https://t.me/RMC_Telegram/42")
    ])
    try:
    ...
    if query.data.startswith("pmfile"):
        ...
        if settings is not None:
            buttons = [
                ...
                [
                    InlineKeyboardButton('Photo Filter',
                                         callback_data=f'setgs#PHOTO#{settings["PHOTO"]}#{str(grp_id)}'),
                    InlineKeyboardButton('✅ 𝖮𝗇' if settings["PHOTO"] else '❌ 𝖮𝖿𝖿',
                                         callback_data=f'setgs#PHOTO#{settings["PHOTO"]}#{str(grp_id)}')
                ],
                [
                    InlineKeyboardButton('VD Filter',
                                         callback_data=f'setgs#VIDEO#{settings["VIDEO"]}#{str(grp_id)}'),
                    InlineKeyboardButton('✅ 𝖮𝗇' if settings["VIDEO"] else '❌ 𝖮𝖿𝖿',
                                         callback_data=f'setgs#VIDEO#{settings["VIDEO"]}#{str(grp_id)}')
                ],
                [
                ...
                [
                    InlineKeyboardButton('PM Filter',
                                         callback_data=f'setgs#BOT_FFILTER#{settings["BOT_FFILTER"]}#{str(grp_id)}'),
                    InlineKeyboardButton('✅ 𝖮𝗇' if settings["BOT_FFILTER"] else '❌ 𝖮𝖿𝖿',
                                         callback_data=f'setgs#BOT_FFILTER#{settings["BOT_FFILTER"]}#{str(grp_id)}')
                ]
            ]
            reply_markup = InlineKeyboardMarkup(buttons)
...
async def auto_filter(client, msg, spoll=False):  
    ...
    if not spoll:
        ...
        if re.findall("((^\/|^,|^!|^\.|^[\u1000-\u109F\U0001F600-\U000E007F]).*)", message.text):
            return
        if len(message.text) < 100:
            search = message.text
            files, offset, total_results = await get_search_results2(message.chat.id ,search.lower(), offset=0, filter=True, photo=settings["PHOTO"], video=settings["VIDEO"])
            if not files:
    ...
    m = await message.reply_sticker("CAACAgIAAxkBAAICL2UDVF11HuhNKx8UgmcjiwPNiDG6AAKzAANwXloPbRKlzjjyhFowBA")
    ...
    if imdb:
        ...
        cap = Config.ADS_1
    cap += f"\n<b>👋 𝖧𝖾𝗒 {message.from_user.mention}\n➡️ သင်ရှာတာက : {search}</b>"
...
async def pmauto_filter(client, msg, spoll=False):
    ...
    m = await message.reply_sticker("CAACAgIAAxkBAAESfnpk0HevDEzmx5DKbqSC3Z8AAf2OzCsAAiMAAygPahQnUSXnjCCkBh4E")
    await asyncio.sleep(1)
    await m.delete()
